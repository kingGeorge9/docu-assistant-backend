...
class TTSService {
  ...
  async textToSpeechAzure(text, options = {}) {
    ...
    speechConfig.speechSynthesisVoiceName = options.voiceName || 'en-US-JennyNeural';
    ...
    return new Promise((resolve, reject) => {
      synthesizer.speakTextAsync(
        ...
        result => {
          if (result) {
            synthesizer.close();
            fs.readFile('output.mp3').then(resolve).catch(reject);
          } else {
            synthesizer.close();
  ...
  async pdfToSpeech(pdfFile, options = {}) {
    try {
      ...
      switch (this.provider) {
        ...
        default:
          throw new Error('No TTS provider configured. Set GOOGLE_APPLICATION_CREDENTIALS, AZURE_SPEECH_KEY, or AWS credentials.');
      }
    } catch (error) {
    ...
    switch (this.provider) {
      ...
      case 'aws':
        return [
          ...
          'Salli', 'Joey', 'Kevin', 'Nicole', 'Russell', 'Amy', 'Brian',
          'Emma', 'Aditi', 'Raveena'
        ];
      default:
        return [];
